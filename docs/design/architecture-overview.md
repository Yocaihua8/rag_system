...
Knowledge Island Web MVP 采用**本地单体分层架构**：FastAPI + Uvicorn 承担本地 HTTP 接口层，SQLite 承担全部持久化，展示层已完成 Vue 3 + Vite 前端工程化收口；B-145 后，后端运行时代码聚合在 `backend/knowledge_island/`，不再保留 legacy 静态前端 fallback。所有处理在本机单进程内完成，无消息队列和微服务。
...
| 层级 | 技术 | 用途 | 选择原因 |
|------|------|------|----------|
| 展示层 | Vue 3 + Vite | 单页应用 UI | B-141A-Z 已完成 Vue 工程骨架和页面级迁移薄片；B-142 已补齐工作台 SSE 与会话历史；B-143 已移除 legacy fallback；B-144 已将构建输出迁至 `backend/knowledge_island/static_dist/` |
...
- `backend/knowledge_island/static_dist/` 是唯一静态前端服务目录，由 `npm run build` 本地生成且不入库
