...
4. 完成并确认 P1 低保真原型，再完成并确认 P2 高保真原型。
5. P2 明确批准后才建立独立 React 前端；通过 OpenAPI 类型连接真实 v3 后端。
...
- P1 只确认结构、流程和状态覆盖，不能解除前端代码门禁。
- P2 必须确认三种主题、视觉 token、文案、键盘、响应式和危险动作。
- 未获 P2 明确批准时，只允许读取现有 Vue 作为事实源和维护 v2 基线；禁止创建 React 生产目录或修改 Vue UI。
