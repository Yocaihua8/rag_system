...
| B-06 | tech-debt | ops/ 运维脚本 | doing | P3 | S | backlog | RAG 团队 | ops/README.md §维护脚本；docs/plans/B-06-ops-maintenance-scripts.md | `ops/scripts/` 目录尚未创建（非空框架）；需新建并实现 `backup_db.sh`（备份 db）、`cleanup_runtime.sh`（清理 runtime）、`rebuild_index.sh`（一键重建索引）。注意 `rebuild_index.sh` 依赖的 `POST /api/admin/rebuild-index` 管理端点当前也不存在，需一并补齐。ops/README 已预先文档化这三个脚本，实现前文档已标注"规划中" |
