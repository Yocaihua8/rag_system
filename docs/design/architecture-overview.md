...
> Related：`system-design-overview.md`、`api-spec.md`、`database-design.md`、`permission-matrix.md`、`../adr/ADR-010-runtime-separation.md`、`../adr/ADR-011-interactive-learning-sql-sandbox.md`
...
FastAPI API（127.0.0.1:8765）
  backend/api + backend/routes
              ...
              ▼
领域服务（backend/domain）
  导入、检索、回答、Coach 分析/评估/逐点学习、Obsidian、只读 Agent 工具
              │ 唯一持久化边界
              ▼
...
| 领域 | `backend/domain/` | 导入、检索、回答、Coach、逐点学习、SQL 练习评分、Agent、Obsidian 规则 | FastAPI Request/Response、页面组件、正式数据库路径注入 SQL 沙箱 |
...
- 当前公开契约共有 90 个唯一 `/api` 路径、98 个 GET/POST 操作（32 GET、66 POST）。
...
- 逐点学习的会话、步骤、exercise 和 attempt 由 `coach_progress_store.py` 持久化；当前覆盖动态合并旧 Coach 评估结果和满足资格的学习 attempt，不新增汇总 mastery 表。
- SQL 练习来源 DDL 只被解析为结构化 fixture。每次评分在独立临时 SQLite 文件中建库并以只读 URI 重开；沙箱不接收 `KnowledgeStore` 连接或正式 `runtime/v2/app.db` 路径。
...
- `CoachLearningSessionOverlay` 从教练、学习地图或确认计划任务打开，不新增主视图键；组件只展示服务端返回的当前步骤、允许动作、评分和计划同步结果。
...
- SQL 练习执行来源 DDL、连接正式应用数据库，或让模型替代确定性查询结果与语义判定。
