...
| B-126 | feature | 知识图谱接入检索流程 | doing | P2 | L | v1.0.0 | RAG 团队 | docs/design/database-design.md, docs/features/graph-enhanced-retrieval.md | `graph_nodes` / `graph_edges` 已建表（B-48），当前检索未使用；补充 Graph-enhanced 检索，扩展关联节点提升多跳推理；预估 5 天；执行 plan：docs/plans/B-126-graph-enhanced-retrieval.md |
