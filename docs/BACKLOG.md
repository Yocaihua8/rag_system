...
> 状态：Active
> Owner：RAG 团队
> Last Updated：2026-05-28（启动 B-143 legacy 静态前端清理）
...
| B-143 | tech-debt | 移除 legacy 静态前端 fallback | doing | P2 | M | v0.12.0 | RAG 团队 | docs/features/frontend-engineering.md, docs/guides/setup.md, docs/guides/testing.md | B-142 已完成；执行计划：docs/plans/B-143-remove-legacy-static-frontend.md；确认 Vue 覆盖旧前端保留主流程后执行：删除 `webapp/static/` legacy 原生前端，调整 `webapp/server.py` 静态文件兜底策略，清理仅针对 legacy 静态前端的测试断言，并同步前端工程、启动和测试文档 |
