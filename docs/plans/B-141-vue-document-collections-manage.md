...
- [x] 浏览器冒烟确认资料库页集合创建/删除入口可见且无控制台错误。
- [x] 相关文档已同步（见下方"回流清单"）。
...
| 内容 | 目标文档 | 是否完成 |
|------|----------|----------|
| B-141M 迁移集合新建/删除，不迁移重命名、加入/移出文档 | `docs/features/frontend-engineering.md` | [x] |
| Vue 文档集合 helper/panel/App 边界 | `docs/design/architecture-overview.md` | [x] |
| Vue 源码测试和浏览器冒烟范围 | `docs/guides/testing.md` | [x] |
| 用户可见变更 | `CHANGELOG.md` | [x] |
| 执行与验证记录 | `docs/devlog/2026-05-27.md` | [x] |
...
- **最后更新**：2026-05-27 15:15
- **进度**：已完成 5 / 5 项（见 § 3 勾选状态）
- **最新 commit**：`6d691c3` — `fix: 更新 Vue 资料库迁移文案`
- **代码状态**：`fix/url-virtual-source-preserve`；工作区存在大量非本片既有未提交改动，本片仅选择性暂存自身文件
- **下一步**：继续 B-141N 页面级业务迁移，可在文档集合重命名/加入移出、Workbench SSE/会话或设置页模型配置中选择下一个薄片
