...
> 状态：Active
> Owner：RAG 团队
> Last Updated：2026-05-28（启动 B-142 Vue 工作台 SSE 与会话历史迁移）
...
| B-142 | feature | Vue 工作台 SSE 与会话历史迁移 | doing | P2 | M | v0.11.0 | RAG 团队 | docs/features/frontend-engineering.md, docs/design/api-spec.md | 从 B-141 非目标拆出：Vue 工作台接入 `/api/answer/stream` EventSource 流式输出、取消当前请求、`/api/chat/sessions*` 与 `/api/chat/messages` 会话列表/历史恢复；不修改后端契约或数据库 schema；预估 3 天；plan：docs/plans/B-142-vue-workbench-sse-sessions.md |
