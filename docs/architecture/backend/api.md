...
> Related：`../overview.md`、`data.md`、`../contracts/permissions.md`、`../decisions/ADR-010-runtime-separation.md`

## 1. 本地 Web MVP HTTP API

当前默认入口为本地 Web MVP：`backend/__main__.py` -> `backend.api.server.run_server()` -> Uvicorn/FastAPI。HTTP 服务默认监听 `http://127.0.0.1:8765`，仅用于本机浏览器访问，不作为远程多用户 API 承诺。FastAPI 自动文档可在本地 `/docs` 查看，OpenAPI 3.0 schema 可在 `/openapi.json` 查看，但正式契约仍以本文档为准。

B-140 起支持可选认证层。默认认证关闭；设置 `RAG_AUTH_ENABLED=1` 后，除 `/api/health`、`/api/auth/token` 和使用独立插件令牌校验的 Obsidian 路由外，所有 `/api/*`、`/docs`、`/redoc`、`/openapi.json` 都需要携带有效应用凭证。凭证支持 `X-API-Key: <key>` 或 `Authorization: Bearer <jwt>`。缺少凭证返回 `401 {"error":"authentication required"}`，凭证错误或过期返回 `401 {"error":"invalid credentials"}`。FastAPI 不托管静态资源，`GET /` 返回 404。

B-168 起前端使用绝对 API URL。后端通过 `KI_CORS_ORIGINS` 精确允许本机 5173/4173 与 Tauri Origin；不启用通配符或 cookie credentials，只允许既有 GET/POST/OPTIONS 和所需认证/内容类型请求头。CORS 不改变任何下述方法、字段或响应契约。
