...
| `frontend/src/api/answer.js` | Vue 工作台问答、工具上下文和回答反馈 API helper，调用既有 `/api/answer`、`/api/answer/stream` 与 `/api/answer/feedback` |
| `frontend/src/api/chat.js` | Vue 工作台聊天会话和历史消息 API helper，调用既有 `/api/chat/sessions*` 与 `/api/chat/messages` 契约 |
...
| `frontend/src/components/` | 迁移期布局组件，例如 `AppShell.vue`、`ProjectSpacePanel.vue`、`QuestionPanel.vue`、`AnswerPanel.vue`、`MarkdownBody.vue`、`ChatSessionPanel.vue`、`SearchDebugPanel.vue`、`AgentToolsPanel.vue`、`DocumentListPanel.vue`、`DocumentPreviewPanel.vue`、`DocumentImportPanel.vue`、`DocumentCollectionPanel.vue`、`ImportBatchHistoryPanel.vue` |
| `frontend/src/views/` | 工作台、资料库、评估、设置等页面组件 |
| `backend/knowledge_island/static_dist/` | Vite 生产构建输出，由 FastAPI 托管 |
...
Vue 3 + Vite 只替代展示层工程组织。后端仍由 `backend/knowledge_island/server.py` 提供 FastAPI 服务，`backend/knowledge_island/api.py` 和 `backend/knowledge_island/routes/*` 保持 API 契约，业务层和数据层不因 B-141 调整。
...
B-150 起，`App.vue` 作为组合层直接复用既有 `apiGet` 读取 `GET /api/projects/summary`，并在项目加载/切换、导入、目录同步、回答完成、工具运行、检索复盘和文档删除后刷新 `appState.projectSummary`。该状态字段已经存在于共享状态模型，本任务不修改 `frontend/src/state/app-state.js`。主题切换逻辑保留在 `AppShell.vue`，只影响 `data-theme` 和浏览器本地偏好，不改变后端配置。

B-151 起，`AnswerPanel.vue` 使用 `MarkdownBody.vue` 渲染回答正文，`MarkdownBody.vue` 不引入第三方依赖，先转义原始文本再生成受控 HTML 标签。`QuestionPanel.vue` 在 emit 提问事件后清空本地输入框。

B-152 起，`WorkbenchView.vue` 作为工作台中列组合层直接渲染历史轮次、流式回答、最新回答、工具建议、工具上下文和底部 composer。`ChatSessionPanel.vue` 不再渲染历史消息块，避免左列和中列重复展示聊天历史。`AppShell.vue` 的项目下拉复用既有 `projects` / `selectedProjectId` props，并通过 `select-project` 事件回到 `App.vue` 的 `handleSelectProject`，不新增共享状态字段。
...
- Vue 工作台回答正文支持 Markdown 标题、列表、引用、行内代码、粗体/斜体和代码块；提问提交后输入框清空。
...
- Vue 首屏 masthead 的 `Vol.` 行在有项目列表时显示项目下拉；切换项目会调用既有项目选择流程。
...
- B-145 完成后，legacy 静态前端 fallback 已删除，后端静态服务目录为 `backend/knowledge_island/static_dist/`，前端静态断言以 Vue 源码测试和构建服务策略测试为准。
