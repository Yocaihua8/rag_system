...
Knowledge Island Web MVP 采用**本地单体分层架构**：FastAPI + Uvicorn 承担本地 HTTP 接口层，SQLite 承担全部持久化，展示层已完成 Vue 3 + Vite 前端工程化收口；B-144 后后端运行时代码聚合在 `backend/webapp/`，不再保留 `webapp/static/` legacy fallback。所有处理在本机单进程内完成，无消息队列和微服务。

| 字段 | 值 |
|------|----|
| 架构模式 | 本地单体（Local Monolith）|
| 核心边界 | 127.0.0.1:8765，不对外暴露 |
| 主要入口 | `backend/app.py` → `backend/webapp/server.py:create_app()` / `run_server()` |
...
┌──────────────────────────────────────────────────────────────┐
│                 浏览器（Vue/Vite 构建产物）                  │
│                 frontend/src/* → backend/webapp/static_dist/         │
└──────────────────────────┬───────────────────────────────────┘
                           │ HTTP REST（127.0.0.1:8765）
┌──────────────────────────▼───────────────────────────────────┐
│           backend/webapp/server.py（FastAPI + Uvicorn）                │
│        静态文件服务 + /api/* JSON + SSE                        │
└──────────────────────────┬───────────────────────────────────┘
                           │
┌──────────────────────────▼───────────────────────────────────┐
│       backend/webapp/api.py + backend/webapp/routes/*（API Handler）          │
...
├──────────┴───────────────┴──────────────┴────────────────────┤
│               backend/webapp/storage.py（SQLite）                     │
...
| 层级 | 技术 | 用途 | 选择原因 |
|------|------|------|----------|
| 展示层 | Vue 3 + Vite | 单页应用 UI | B-141A-Z 已完成 Vue 工程骨架和页面级迁移薄片；B-142 已补齐工作台 SSE 与会话历史；B-143 已移除 legacy fallback；B-144 已将构建输出迁至 `backend/webapp/static_dist/` |
...
- `frontend/` 是 B-141 起的 Vue 3 + Vite 源码目录，生产构建输出到 `backend/webapp/static_dist/`
...
- `backend/webapp/static_dist/` 是唯一静态前端服务目录，由 `npm run build` 本地生成且不入库
- 接收用户操作，通过 `fetch` 调用后端 REST API
- 管理客户端 UI 状态（当前项目、会话、文档列表）
- 渲染回答、来源、检索结果、工具输出
- 必须：所有业务规则不在前端实现，前端只做展示和 API 调用

### 4.2 接口层（backend/webapp/server.py + backend/webapp/api.py + backend/webapp/routes/*）

- `backend.webapp.server.create_app()` 创建 FastAPI app，`run_server()` 通过 Uvicorn 启动本地服务
- `backend.webapp.api.dispatch()` 保持兼容入口，解析 `raw_path` 后交给 `backend.webapp.routes.dispatch_to_routes()`
- `/api/answer/stream` 由 FastAPI `StreamingResponse` 输出既有 SSE 事件
- 静态前端服务 `backend/webapp/static_dist/`；构建产物不存在时返回 503 构建提示，不再回退 legacy 前端
- `backend/webapp/routes/*` 按领域承载 REST 路由分支，提取 URL 参数和请求体
...
### 4.4 数据层（backend/webapp/storage.py）
...
| 能力 | 实现 | 调用方 |
|------|------|--------|
| HTTP 服务 | `backend.webapp.server.create_app` / `backend.webapp.server.run_server` | `backend/app.py` / Uvicorn |
| API 分发 | `backend.webapp.api.dispatch` + `backend.webapp.routes.dispatch_to_routes` | `backend.webapp.server` |
| SQLite 存储 | `KnowledgeStore` | `backend.webapp.routes/*`、导入/检索/工具模块 |
...
| Node.js / npm | 必需前端构建工具 | 安装 Vue/Vite 依赖并生成 `backend/webapp/static_dist/` | 未构建时 FastAPI 返回 503 构建提示 |
...
| Docker | 可选 | 容器化一键启动 | 非必需；`python backend/app.py` 是主要入口 |
...
| 方案 | 是否采用 | 原因 |
|------|----------|------|
| FastAPI 替代 `http.server` | 已采用（B-139）| ADR-001；保留 `backend.webapp.api.dispatch()` 兼容入口，先迁移 HTTP 外壳，B-140/B-141 串行推进 |
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-144 已解耦前后端目录）| ADR-006；B-141 已完成工程骨架和主要页面级入口迁移，B-142 已补齐 Workbench SSE/会话，B-143 已删除 `webapp/static/`，B-144 已将后端聚合到 `backend/webapp/` |
...
| BM25 替代 regex 关键词检索 | 已采用（B-127）| `backend/webapp/search.py` 使用内置 BM25 计算 `keyword_score`，不新增必需依赖 |
| `api.py` 按领域拆分 | 已完成（B-138）| 61 个 REST 端点已迁入 `backend/webapp/routes/*`；`backend/webapp/api.py` 仅保留 `dispatch()`、`answer_stream_events()` 和兼容导出入口，保持 `backend.webapp.api.dispatch` 兼容入口 |
