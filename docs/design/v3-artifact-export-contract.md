...
| POST | `/api/v3/artifacts/{artifact_id}/export-confirm` | 用户明确确认后，核对 `expected_checksum` 与 `expected_version`，将固定快照原子写入受管 v3 导出目录 |
...
- 确认时把 Artifact 状态转为 `exported`、写入 `content_ref` 与 `exported_at`；失败保持原 Artifact 可读状态并清理仅由本次请求创建的 staging 文件。
