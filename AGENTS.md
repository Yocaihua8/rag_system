...
| 项 | 值 |
|----|----|
| 项目类型 | Web 全栈（FastAPI + Uvicorn 后端 + 原生 HTML/CSS/JS 前端） |
| 当前阶段 | MVP |
...
| 前端 | 原生 HTML/CSS/JS | — | 不引入构建工具或 JS 框架 |
| 测试框架 | pytest | 7+ | — |
...
- 表现层：`webapp/server.py`（FastAPI app、静态文件、SSE）、`webapp/api.py`（兼容分发、参数校验）
...
- 前端 JS 按职责拆分：`api.js` / `state.js` / `ui.js` / `projects.js` / `answer.js`
