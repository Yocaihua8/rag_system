...
- [ ] 增加 Vue 源码契约红灯测试，覆盖设置 API helper、设置页模型表单/Profile 列表和 App 状态流。
- [ ] 实现 `frontend/src/api/settings.js`、`SettingsView.vue`、`App.vue` 和共享状态的最小模型设置/Profile 串联。
...
| 内容 | 目标文档 | 是否完成 |
|------|----------|----------|
| B-141R 用户可见行为、非目标和验收标准 | `docs/features/frontend-engineering.md` | [ ] |
| Vue 设置页 API helper、状态和组件职责 | `docs/design/architecture-overview.md` | [ ] |
| Vue 设置页模型配置测试要求 | `docs/guides/testing.md` | [ ] |
| Unreleased 变更记录 | `CHANGELOG.md` | [ ] |
...
- **最新 commit**：`待提交` — docs: 创建 B-141R 设置页迁移计划
- **代码状态**：`fix/url-virtual-source-preserve`；工作区存在多项用户/历史未提交改动，本片仅允许暂存 B-141R 相关文件
- **下一步**：增加 Vue 源码契约红灯测试，覆盖设置 API helper、设置页模型表单/Profile 列表和 App 状态流
