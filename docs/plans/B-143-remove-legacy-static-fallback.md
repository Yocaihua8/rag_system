...
- [ ] 任务 2：修改 `webapp/server.py`，移除 `STATIC_LEGACY_DIR` / `STATIC_DIR` / fallback 逻辑。
...
- **最后更新**：2026-06-26 18:42
- **进度**：已完成 1 / 6 项（见 § 3 勾选状态）
- **最新 commit**：待提交 — 任务 1 红灯测试完成后立即提交
- **代码状态**：有未提交 B-142 改动；新增 B-143 plan；未跟踪 `docs/design/new-architecture-design.md` 不属于本任务
- **下一步**：任务 2：修改 `webapp/server.py`，移除 legacy 静态 fallback
- **续任务须知**：任务 1 已确认红灯；下一步只改 `webapp/server.py` 静态目录选择逻辑，不触碰 API、数据库、Vue 源码、`backend/` 或 `webapp/search.py`。
