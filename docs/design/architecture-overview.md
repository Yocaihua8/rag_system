...
- `frontend/src/api/imports.js` 封装 Vue 资料库文本笔记和 URL 摘录导入入口，调用既有 `POST /api/import/note` 与 `POST /api/import/url` 契约
...
- `frontend/src/components/DocumentImportPanel.vue` 是 B-141F 的资料库轻量导入薄片，负责文本笔记和 URL 摘录两个导入表单
...
| Vue 轻量导入 helper | `frontend/src/api/imports.js` | `App.vue` / `DocumentImportPanel.vue` |
...
| Vue 3 / Vite | 必需前端构建依赖 | B-141 起的前端工程化和生产构建 | B-141A 提供骨架，B-141C/D/E/F 已迁移项目空间、非流式问答、文档浏览和轻量导入薄片；完整业务 UI 仍在迁移中 |
...
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-141A 起分阶段迁移）| ADR-006；先建立工程骨架和构建链，B-141C/D/E/F 已迁移项目空间、非流式问答、文档浏览和轻量导入薄片，完整业务 UI 后续按页面迁移 |
