...
> Related：`../../architecture/backend/api.md`、BACKLOG B-136
...
- 覆盖当前 `docs/architecture/backend/api.md` 中列出的 Web MVP API。
...
正式接口契约仍维护在 `docs/architecture/backend/api.md`。OpenAPI schema 是运行时可视化和联调辅助入口，必须与该文档同步更新。
...
- `POST` operation 使用通用 JSON object requestBody；复杂响应也先以 object schema 表达，具体字段见 `docs/architecture/backend/api.md`。
