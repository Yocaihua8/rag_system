...
| 项 | 值 |
|----|----|
| 项目类型 | Web 全栈（FastAPI + Uvicorn 后端 + Vue 3/Vite 前端） |
| 当前阶段 | MVP |
...
| 前端 | Vue 3 + Vite | Vue 3.5+ / Vite 7+ | B-145 后只服务 `backend/knowledge_island/static_dist/` 构建产物，不再保留 legacy fallback |
| 测试框架 | pytest | 7+ | — |
...
| 路径 | 用途 |
|------|------|
| `backend/` | FastAPI 后端运行时代码根目录 |
| `backend/knowledge_island/` | Web MVP 后端模块（API、检索、回答、导入、存储） |
| `frontend/` | Vue 3 + Vite 前端工程源码（B-141 起） |
| `ops/docker/` | Dockerfile、Compose 与一键启停脚本 |
| `legacy/desktop/` | Legacy PySide6 桌面端（六边形架构，历史参考） |
| `tests/` | 测试代码根目录 |
...
| `docs/template-mapping.md` | 历史文档与当前结构映射 |
...
.venv\Scripts\python.exe backend/app.py   # 启动 Web MVP（默认 http://127.0.0.1:8765）
```
...
本项目默认入口为 Web MVP，采用**三层架构**；Legacy（`legacy/desktop/`）采用六边形架构，作为历史参考保留。

**Web MVP 三层职责**：

- 表现层：`frontend/`（Vue 3 + Vite 工程源码）、`backend/knowledge_island/static_dist/`（本地构建产物，不入库）、`backend/knowledge_island/server.py`（FastAPI app、静态文件、SSE）、`backend/knowledge_island/api.py`（兼容分发、参数校验）
- 业务层：`backend/knowledge_island/answers.py`（回答生成）、`backend/knowledge_island/search.py`（检索）、`backend/knowledge_island/ingestion.py`（导入管线）、`backend/knowledge_island/agent_tools.py`（Agent 工具）
- 数据层：`backend/knowledge_island/storage.py`（SQLite 唯一读写入口）

**关键边界**：

- `api.py` 只做参数校验和用例编排，不直接操作 SQLite
- `storage.py` 是 SQLite 唯一入口，不承载业务规则
- Agent 工具只允许只读操作，白名单硬编码在 `backend/knowledge_island/agent_tools.py`（ADR-003）
...
- 每个 `backend/knowledge_island/` 模块文件只承载一个清晰职责（api / storage / search / answers / ingestion 各管一件事）
- 可选依赖（pymupdf / jieba）必须用 `try/except ImportError` 引入，失败时提供明确降级
- Vue 前端源码放在 `frontend/src/`，生产构建输出到 `backend/knowledge_island/static_dist/`，不得提交 `node_modules/` 或构建产物
...
| 类型 | 路径模式 | 运行命令 |
|------|----------|----------|
| Web MVP 单元/集成 | `tests/backend/`、`tests/frontend/` | `.venv\Scripts\python.exe -m pytest tests/backend tests/frontend -q` |
...
测试优先级：API 契约 > 核心业务规则（检索 / 回答 / 导入） > 数据层集成 > Vue 源码与前端构建入口。
...
- 修改 `backend/knowledge_island/` 下的业务代码与测试
- 按上方"文档联动规则"同步文档
...
- 删除或重命名已有公共 API 接口
- 修改数据库 Schema（`backend/knowledge_island/storage.py` 中的 `CREATE TABLE` / `ALTER TABLE`）
- 变更 Agent 工具权限逻辑（白名单内容）
- 新增 ADR（需和用户确认决策内容）
- 修改 `legacy/desktop/`（Legacy）代码
- 向主分支直接推送
...
- 批量删除 `legacy/desktop/` 或 `docs/archive/` 等历史目录（确认归档内容已迁移前）
