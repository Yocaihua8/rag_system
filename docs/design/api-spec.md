...
| `quality_metrics` | 回答质量指标，包括来源覆盖率、最高检索分、是否有来源和回答长度 |
| `parent_message_id` | 分支来源消息 ID；普通消息为空 |
| `branch_id` | 分支 ID；普通消息为 `main` |
| `is_active` | 是否属于当前活跃分支 |
| `created_at` | 记录时间 |
...
| 方法 | 路径 | 请求 | 成功响应 | 错误 |
|------|------|------|----------|------|
| GET | `/api/admin/stats` | N/A | `{"db_size_bytes":0,"chunk_count":0,"vector_count":0,"project_count":0}` | N/A |
| POST | `/api/admin/rebuild-index` | N/A | `{"status":"rebuild complete"}` | N/A |

`/api/admin/stats` 只读取本地 SQLite 与项目统计，返回数据库文件大小、项目数、chunk 总数和 SQLite `chunk_vectors` 总数。`/api/admin/rebuild-index` 会遍历所有项目 chunk，使用本地 `hashing-96` 重新计算向量并写入当前 `VectorBackend`；该接口不重新导入文档、不删除 SQLite 记录，也不调用外部 LLM。
