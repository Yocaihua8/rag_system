...
- `ops/scripts/backup_db.sh` 可备份当前 SQLite 数据库，并可在启用 Qdrant local 目录时一并打包向量存储目录。
- `ops/scripts/cleanup_runtime.sh` 只清理 runtime 下的临时文件、缓存和 Python 字节码，不删除 SQLite 数据库、备份、Qdrant 数据目录或用户导入内容。
- `ops/scripts/rebuild_index.sh` 调用本地 `POST /api/admin/rebuild-index`，由后端基于 SQLite 已存文档内容重建 chunk 与向量索引。
...
维护脚本位于 `ops/scripts/`，通过环境变量接收路径和服务地址。后端维护入口位于 `webapp/routes/admin.py`，由 `webapp.routes.dispatch_to_routes()` 分发；路由层只做参数解析和 JSON 响应封装，索引重建能力由 `KnowledgeStore` 暴露的数据维护方法完成。
