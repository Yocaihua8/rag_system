...
- [x] 同步功能文档、架构/测试/devlog/CHANGELOG 中的 B-141C 说明
- [x] 运行 npm 构建、Vue 前端工程测试、Web MVP 全量、legacy 回归和页面非空检查
- [x] 更新 plan 快照，保留 B-141 为 `doing`，下一步指向 B-141D 页面迁移
...
- [x] Vue 资料库视图能显示项目空间列表、当前项目名称和本地目录状态。
- [x] Vue 项目空间选择会写入共享状态并持久化最近项目 ID。
- [x] Vue 新建项目表单调用现有 `POST /api/projects`，成功后刷新列表并选中新项目。
- [x] B-141C 不迁移导入、重命名、删除、文档列表、问答、评估或设置业务流程。
- [x] `npm run build` 通过。
- [x] 相关测试与文档同步完成。
...
| 内容 | 目标文档 | 是否完成 |
|------|----------|----------|
| B-141C 项目空间迁移边界 | `docs/features/frontend-engineering.md` | [x] |
| Vue 项目空间 helper/组件架构说明 | `docs/design/architecture-overview.md` | [x] |
| Vue 项目空间 source 测试要求 | `docs/guides/testing.md` | [x] |
...
- 2026-05-26：同步功能文档、架构、测试指南、CHANGELOG 和 devlog 中的 B-141C 项目空间说明。
- 2026-05-26：验证通过：`npm run build`、Vue 前端工程测试、Web MVP 全量、legacy 回归、docs contract、`git diff --check`；Chrome CDP 点击资料库后确认项目空间面板非空。
- 2026-05-26：B-141C 已完成；BACKLOG B-141 保持 `doing`，下一步进入 B-141D 页面级业务迁移。

## 9. 状态快照

- **最后更新**：2026-05-26 17:23
- **进度**：已完成 8 / 8 项（见 § 3 勾选状态）
- **最新 commit**：`cd9d974` — docs: 记录 B-141C 验证结果
- **代码状态**：分支 `fix/url-virtual-source-preserve`；存在大量既有未提交改动；B-141C 相关变更已完成
- **下一步**：B-141D：选择下一个 Vue 页面级业务切片，建议在资料库导入/文档列表与工作台问答入口之间择一
- **续任务须知**：B-141D 仍不得删除 `webapp/static/`，不得迁移无关业务流程，不修改数据库 schema，不新增 Pinia/Vue Router
