...
v0.9.0 完成了 `webapp/api.py` 的领域路由拆分（B-138），路由层与业务层边界已清晰，**迁移窗口已到，且当前 P1 BACKLOG 已清空**。
...
- `webapp/storage.py` 保持不变
- `webapp/search.py`、`webapp/answers.py`、`webapp/ingestion.py` 等业务层保持不变
- 仅替换路由层（`webapp/routes/*.py`）与服务器启动逻辑（`webapp/server.py`、`app.py`）
- 前后端分离完成前，`webapp/static/` 临时通过 `StaticFiles` 挂载继续服务

## 3. 决策原因

1. **与 storage.py 无缝兼容**：FastAPI 无 ORM，现有裸 SQLite 层零改动
2. **原生 async + SSE**：`async def` 路由 + `StreamingResponse` 替换手写 chunked，代码量减少约 60%
3. **路由迁移成本最低**：`webapp/routes/*` 已按领域拆分（B-138），一对一映射为 FastAPI Router
...
| 模块 | 变更内容 |
|------|----------|
| `webapp/server.py` | 替换为 FastAPI `app` 实例 + Uvicorn 启动逻辑 |
| `webapp/api.py` | `dispatch()` 逻辑迁移为 FastAPI Router；`answer_stream_events` 改为 `StreamingResponse` |
| `webapp/routes/*.py` | 路由函数签名改为 `async def` + Pydantic 请求体 |
| `app.py` | 入口改为 `uvicorn webapp.main:app --host ... --port ...` |
| `requirements.txt` | 新增 `fastapi`、`uvicorn[standard]` |
| `webapp/static/` | 不变；迁移期间通过 `StaticFiles` 挂载，前后端分离后移除 |
...
| 回滚步骤 | `git revert` 迁移相关 commits；恢复 `webapp/server.py` 与 `app.py` 原实现；回退 `requirements.txt` |
