...
当前 Web MVP 导入时会把文档拆成 SQLite `document_chunks`，检索按 chunk 片段召回并排序，不再把整篇文档作为最小召回单位。`hits` / `sources` 中每条结果包含 `path`、`document_id`、`snippet`、`score`，命中 chunk 时还包含 `chunk_id`、`chunk_index`。问答在配置 `RAG_LLM_PROVIDER=api` 且存在 `RAG_LLM_API_KEY` / DeepSeek Key 别名时，优先请求 OpenAI-compatible Chat Completions；未配置或请求失败时回退到本地命中片段组合回答。无命中时不伪造来源。
