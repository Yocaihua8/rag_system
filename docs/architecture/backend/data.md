...
> Related：docs/architecture/overview.md, docs/architecture/decisions/ADR-008-project-knowledge-coach-v2.md, docs/architecture/decisions/ADR-009-obsidian-plugin-bridge.md
...
在概念分组中，`coach_assessments` 由 `coach_assessment_*` 四张评估表共同实现；`learning_plans` 由 `coach_learning_plans` 与 `coach_learning_plan_items` 共同实现。这里的概念名称用于文档检索，不代表额外的 SQLite 表。
