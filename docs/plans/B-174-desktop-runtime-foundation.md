...
- [ ] 记录桌面临时端口、进程期令牌和 Web 认证分界 ADR，冻结默认关闭与回滚规则。
- [ ] 实现后端桌面模式配置、精确环回绑定、会话令牌校验和兼容认证测试。
...
| 内容 | 目标文档 | 是否完成 |
|------|----------|----------|
| 桌面端口/令牌/认证边界 | `../adr/`、`../design/permission-matrix.md` | [ ] |
...
- **最后更新**：2026-08-03 09:45:43 +08:00
- **进度**：已完成 0 / 7 项（见 § 3 勾选状态）
- **最新 commit**：`f421d05` — 回流 B-175 React 验收状态
- **代码状态**：`refactor/agent-v3`；治理文件产生未提交改动，尚未修改运行时代码
- **下一步**：建立桌面端口、令牌和 Web 认证分界 ADR，再实现后端默认关闭的 desktop mode
