...
- [ ] 增加 Vue 源码契约红灯测试，覆盖评估 API helper、评估页 UI 和 App 状态流。
- [ ] 实现 `frontend/src/api/assessment.js`、`AssessmentView.vue`、`App.vue` 和共享状态的评估闭环串联。
...
- **最新 commit**：`待提交` — docs: 创建 B-141T 评估页迁移计划
- **代码状态**：`fix/url-virtual-source-preserve`；工作区存在多项用户/历史未提交改动，本片仅允许暂存 B-141T 相关文件
- **下一步**：增加 Vue 源码契约红灯测试，覆盖评估 API helper、评估页 UI 和 App 状态流
