...
把已写入 SQLite 的 `agent_tool_runs` 审计记录暴露为只读接口，并在工作台展示当前项目工具运行历史，便于确认工具名、状态、参数和失败原因。

### 变更文件

| 操作 | 路径 | 内容 |
|------|------|------|
| 更新 | `webapp/api.py` | 新增 `GET /api/agent/tools/runs?project_id=...`，按最近优先返回当前项目工具运行记录 |
| 更新 | `webapp/static/index.html`、`webapp/static/js/agent.js`、`webapp/static/js/app.js`、`webapp/static/js/state.js`、`webapp/static/js/ui.js`、`webapp/static/styles.css` | 工作台新增工具运行历史列表，工具执行或切换项目后刷新 |
| 更新 | `tests/test_webapp/test_agent_tools.py`、`tests/test_webapp/test_frontend_static.py`、`tests/test_webapp/test_docs_contract.py` | 覆盖历史接口、缺少项目参数、未知项目、前端接线和文档契约 |
| 更新 | `README.md`、`docs/design/api-spec.md`、`docs/guides/setup.md`、`docs/guides/testing.md`、`CHANGELOG.md`、`docs/BACKLOG.md` | 同步工具运行历史边界 |

### 关键行为

- 历史接口只读，不开放工具结果修改或清理。
- 前端按当前项目展示最近运行记录；项目切换后重新加载。
- 工具成功和失败记录都会展示，方便追踪参数与错误。

## 2026-05-22 | B-73 — Web 回答建议工具手动运行

### 目标

让 B-72 的 `tool_suggestion` 从纯文本提示变成可操作入口：用户明确点击后才运行只读 `search_sources`，保持 Agent 工具不会自动执行。

### 变更文件

| 操作 | 路径 | 内容 |
|------|------|------|
| 更新 | `webapp/static/index.html` | 回答卡片新增 `apply-tool-suggestion-button` |
| 更新 | `webapp/static/js/state.js`、`webapp/static/js/app.js`、`webapp/static/js/ui.js` | 保存当前工具建议，显示/隐藏手动运行按钮，点击后调用 `runAgentTool("search_sources")` |
| 更新 | `webapp/static/styles.css` | 增加建议工具按钮布局 |
| 更新 | `tests/test_webapp/test_frontend_static.py`、`tests/test_webapp/test_docs_contract.py` | 覆盖建议按钮、状态接线、手动运行边界和文档说明 |
| 更新 | `README.md`、`docs/design/api-spec.md`、`docs/guides/setup.md`、`docs/guides/testing.md`、`CHANGELOG.md`、`docs/BACKLOG.md` | 同步手动运行边界 |

### 关键行为

- 无来源回答显示建议工具按钮；有来源回答不显示。
- 用户点击按钮后才运行 `search_sources`，并把结果渲染到 Agent 工具结果区。
- 按钮只支持当前白名单工具 `search_sources`，不扩大 Agent 权限。
