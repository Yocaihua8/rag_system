...
- 变更 Vue API helper、项目空间 helper、共享状态、基础布局组件、项目空间组件或 Vue 主视图壳时，必须覆盖 `tests/test_webapp/test_frontend_vue_app.py` 并运行 `npm run build`。
...
- Vue 前端包含 API client、共享状态模型和工作台 / 资料库 / 评估 / 设置基础视图壳；完整业务流程未迁移前仍由 legacy 静态前端承担
