...
> Related：docs/architecture/backend/api.md, docs/architecture/backend/data.md, docs/product/features/project-space-ingestion.md, docs/product/features/project-knowledge-coach.md, docs/BACKLOG.md B-137 / B-163
...
其中配对完成、事件同步、待执行发布和结果回传由配对码或插件 Bearer 令牌鉴权；连接查看、主应用撤销以及发布预览/确认仍使用应用自身认证边界。插件也可用 Bearer 令牌撤销自身连接，但不能撤销其他连接。完整字段和错误契约见 `docs/architecture/backend/api.md`。
