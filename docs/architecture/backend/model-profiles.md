...
> Related：`api.md`、`data.md`、`../../product/features/multi-model-comparison.md`
...
- B-111 不建表、改接口实现或改变当前 `.env` 写入逻辑；B-112 已新增 `model_profiles` 表和最小接口。
- 不把 API Key 明文存入 `model_profiles` 表；如必须保存 Key，只允许沿用当前 appdata `.env` 或后续受控凭证引用方案。

## 4. 数据模型建议

B-112 已新增 `model_profiles` 表：
