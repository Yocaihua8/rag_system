...
| Job | 内容 | 触发缓存 key |
|-----|------|-------------|
| `python-tests` | `npm ci` + `npm run build` + `pytest tests/test_backend tests/test_webapp -q` + `scripts/check_docs_consistency.py` | Python 版本 + `requirements.txt` + `requirements-dev.txt` hash；Node 版本 + `package-lock.json` hash |
