...
- `frontend/src/api/imports.js` 封装 Vue 资料库文本笔记、URL 摘录、普通文件上传和导入批次历史入口，调用既有 `POST /api/import/note`、`POST /api/import/url`、`POST /api/import/upload`、`GET /api/import/batches` 与 `GET /api/import/batches/detail` 契约
...
- `frontend/src/components/DocumentImportPanel.vue` 是 B-141F/H 的资料库导入薄片，负责文本笔记、URL 摘录和普通文件上传入口
...
| Vue 3 / Vite | 必需前端构建依赖 | B-141 起的前端工程化和生产构建 | B-141A 提供骨架，B-141C/D/E/F/G/H 已迁移项目空间、非流式问答、文档浏览、轻量导入、批次历史和普通文件上传薄片；完整业务 UI 仍在迁移中 |
...
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-141A 起分阶段迁移）| ADR-006；先建立工程骨架和构建链，B-141C/D/E/F/G/H 已迁移项目空间、非流式问答、文档浏览、轻量导入、批次历史和普通文件上传薄片，完整业务 UI 后续按页面迁移 |
