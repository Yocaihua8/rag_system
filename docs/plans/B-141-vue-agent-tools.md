...
- [x] 功能行为符合 `docs/features/frontend-engineering.md` 的 B-141W 业务规则。
- [x] Vue 工作台展示 `project_overview` 和 `search_sources` 只读工具元数据。
- [x] Vue 工作台可运行 `project_overview` 并展示项目、文档、分块、向量和对话统计。
- [x] Vue 工作台可输入 query 运行 `search_sources` 并展示命中数量和来源片段。
- [x] Vue 工作台展示当前项目工具运行历史，并可读取单条运行详情。
- [x] Agent helper 只调用既有 `/api/agent/tools*` 契约，不修改工具白名单、不自动执行工具、不修改数据库 schema。
...
- 2026-05-28：完成最终验证：Web MVP 全量 `tests/test_webapp` 为 332 passed，legacy 回归 `tests/test_application tests/test_domain tests/test_adapters` 为 179 passed，`npm run build` 成功；headless Chrome 冒烟通过 Agent 工具面板、`project_overview`、`search_sources`、运行历史和详情展示，业务 console/runtime error 数 0，业务网络错误数 0。烟测临时项目 `B-141W-smoke-20260528022511` 已删除，本地服务已停止。

## 9. 状态快照

- **最后更新**：2026-05-28 02:32
- **进度**：已完成 6 / 6 项（见 § 3 勾选状态）
- **最新 commit**：待提交 — docs: 更新 B-141W 验证快照
- **代码状态**：`fix/url-virtual-source-preserve`；工作区存在多项用户/历史未提交改动，本片仅允许暂存 B-141W 相关文件
- **下一步**：B-141 后续可继续迁移 Workbench SSE/会话、回答区工具建议/来源回填、项目级检索默认值/复盘，或评估题库/历史列表
