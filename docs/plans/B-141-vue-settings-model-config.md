...
- [x] Vue 设置页可读取、保存和测试基础模型设置，且不回显 API Key 明文。
- [x] Vue 设置页可读取模型 Profile 列表，创建/编辑/删除 Profile，设置/清空默认 Profile，并测试单个 Profile。
- [x] B-141R 不迁移 Prompt 预设、Workbench SSE/会话、项目根目录修改、批量项目管理或数据库 schema。
- [x] Vue 源码契约测试通过。
- [x] `npm run build` 通过。
...
- 2026-05-27：完成 B-141R 验证：Vue 源码测试 46 passed，Vite build 成功，Web MVP 全量 317 passed，legacy 回归 179 passed；浏览器烟测确认设置页模型配置入口可见、API Key 输入为空且控制台 error 数 0。

## 9. 状态快照

- **最后更新**：2026-05-27 00:00
- **进度**：已完成 6 / 6 项（见 § 3 勾选状态）
- **最新 commit**：`f772829` — docs: 同步 B-141R 设置页迁移说明
- **代码状态**：`fix/url-virtual-source-preserve`；工作区存在多项用户/历史未提交改动，本片仅允许暂存 B-141R 相关文件
- **下一步**：继续 B-141S 页面级业务迁移，可在 Workbench SSE/会话、设置页 Prompt 预设或评估页中选择下一个薄片
