...
|----|------|------|------|--------|------|--------|----------|------|
| B-169 | fix | 对齐 v2 数据库备份与结果导出路径 | doing | P1 | S | RAG 团队 | `docs/plans/B-169-v2-operations-paths.md` | 从旧冲突分支语义移植 ISSUE-006 路径修复：保留当前运行时结构，仅补 Windows SQLite 在线备份兼容、真实隔离恢复测试和配置层结果输出目录；chunk/retriever/top-k 接线不在本任务范围。 |
...
- **计划处理方式**：B-169 仅修复数据库备份与结果导出路径并补真实恢复测试；chunk/retriever/top-k 配置接线继续独立跟踪，不在本任务中改变。
