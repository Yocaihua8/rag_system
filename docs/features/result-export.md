...
- `webapp/routes/export.py` 承载 `POST /api/export/result` 参数校验、项目存在性校验和消息归属校验。
- `webapp/result_export.py` 负责 Markdown / PDF 内容生成、输出目录解析、文件写入和元数据返回。
- `webapp/storage.py` 不新增表结构；导出读取既有 `chat_messages` 记录。
