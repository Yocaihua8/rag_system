...
- [ ] 补齐 Windows SQLite 在线备份路径转换，并增加真实脚本执行、隔离恢复和数据完整性测试。
- [ ] 让结果导出默认使用配置层 `outputs_dir`，保留显式环境变量覆盖优先级并补回归测试。
...
- **最后更新**：2026-08-01 14:38（Asia/Shanghai）
- **进度**：已完成 0 / 4 项（见 § 3 勾选状态）
- **最新 commit**：`26b02b7` — refactor: 重构仓库运行时与文档治理 (#5)
- **代码状态**：`agent/b-169-v2-paths`；仅包含待提交的 BACKLOG 与 plan 启动改动
- **下一步**：补齐 Windows SQLite 在线备份兼容与真实恢复测试
