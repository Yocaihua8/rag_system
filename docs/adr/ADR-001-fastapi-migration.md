...
v0.9.0 完成了 API 领域路由拆分（B-138），路由层与业务层边界已清晰，**迁移窗口已到，且当前 P1 BACKLOG 已清空**。B-155 后当前实现路径为 `backend/api/dispatch.py` 与 `backend/routes/*`。
...
3. **路由迁移成本最低**：领域路由已按职责拆分（B-155 后位于 `backend/routes/*`），一对一映射为 FastAPI Router
