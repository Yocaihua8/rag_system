...
- `frontend/src/api/document-collections.js` 封装 Vue 资料库文档集合列表、新建、重命名和删除入口，调用既有 `GET/POST /api/document-collections`、`POST /api/document-collections/update` 与 `POST /api/document-collections/delete` 契约
...
| Vue 3 / Vite | 必需前端构建依赖 | B-141 起的前端工程化和生产构建 | B-141A 提供骨架，B-141C/D/E/F/G/H/I/J/K/L/M/N 已迁移项目空间、非流式问答、文档浏览、轻量导入、批次历史、普通文件上传、浏览器文件夹上传、当前目录同步、导入预检和文档集合筛选/新建/删除/重命名薄片；完整业务 UI 仍在迁移中 |
...
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-141A 起分阶段迁移）| ADR-006；先建立工程骨架和构建链，B-141C/D/E/F/G/H/I/J/K/L/M/N 已迁移项目空间、非流式问答、文档浏览、轻量导入、批次历史、普通文件上传、浏览器文件夹上传、当前目录同步、导入预检和文档集合筛选/新建/删除/重命名薄片，完整业务 UI 后续按页面迁移 |
