...
from backend.webapp.api import answer_stream_events, dispatch
from backend.webapp.auth import AuthSettings, issue_jwt, load_auth_settings, validate_api_key, validate_jwt
from backend.webapp.config import DEFAULT_HOST, DEFAULT_PORT, default_db_path
    ...
    async def frontend_build_missing(path: str = "") -> HTMLResponse:
        return HTMLResponse(
            ...
            content=(
                ...
                "生成 <code>backend/webapp/static_dist/index.html</code> 后再启动 Web MVP。</p>"
                "</body></html>"
            ),
        )
