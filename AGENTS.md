...
| 前端 | Vue 3 + Vite | Vue 3.5+ / Vite 7+ | B-143 后只服务 `webapp/static_dist/` 构建产物，不再保留 legacy fallback |
| 测试框架 | pytest | 7+ | — |
...
- 表现层：`frontend/`（Vue 3 + Vite 工程源码）、`webapp/static_dist/`（本地构建产物，不入库）、`webapp/server.py`（FastAPI app、静态文件、SSE）、`webapp/api.py`（兼容分发、参数校验）
