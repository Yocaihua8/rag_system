...
- [x] 迁移后端目录与启动链路，更新 imports、Vite 构建输出、Docker COPY/CMD 和测试引用。
...
- **最后更新**：2026-05-28 16:05
- **进度**：已完成 2 / 3 项（见 § 3 勾选状态）
- **最新 commit**：`待本次提交生成` — 迁移后端目录与启动链路
- **代码状态**：fix/b-142-vue-workbench-sse-sessions；有未提交目录迁移、测试和 plan 更新
- **下一步**：同步正式文档，更新 BACKLOG/CHANGELOG/devlog，完成全量验证并删除本 plan
- **续任务须知**：迁移后 `backend.webapp.config.runtime_dir()` 必须继续指向仓库根 `runtime/webapp`，不能落到 `backend/runtime`；聚焦验证 18 passed，Web MVP 全量验证 285 passed。
