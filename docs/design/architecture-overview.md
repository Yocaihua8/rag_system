...
Knowledge Island Web MVP 采用**本地单体分层架构**：FastAPI + Uvicorn 承担本地 HTTP 接口层，SQLite 承担全部持久化，展示层已完成 B-141 Vue 3 + Vite 前端工程化收口；`webapp/static/` 仍保留为未迁移高级交互和回滚 fallback。所有处理在本机单进程内完成，无消息队列和微服务。
...
| 层级 | 技术 | 用途 | 选择原因 |
|------|------|------|----------|
| 展示层 | Vue 3 + Vite / legacy Vanilla HTML/CSS/JS | 单页应用 UI | B-141A-Z 已完成 Vue 工程骨架和页面级迁移薄片；`webapp/static/` 仍作为 fallback |
...
- `webapp/static/` 是 legacy 原生前端 fallback，B-141 收口后仍保留
...
| Vue 3 / Vite | 必需前端构建依赖 | B-141 起的前端工程化和生产构建 | B-141A-Z 已完成工程骨架、项目空间选择/创建/改名/删除、非流式问答、回答反馈、检索调试、项目级检索默认值、检索复盘、Agent 只读工具、工具来源上下文、文档浏览/删除、轻量导入、批次历史、普通文件上传、浏览器文件夹上传、当前目录同步、导入预检、文档集合筛选/新建/删除/重命名/加入/移出、设置页模型配置/Prompt 预设和评估页最小闭环薄片；Workbench SSE/会话等后续能力由 B-142 等条目跟进 |
...
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-141A-Z 已收口）| ADR-006；B-141 已完成工程骨架和主要页面级入口迁移，`webapp/static/` 保留为 fallback；Workbench SSE/会话等高级交互拆为 B-142 等后续条目 |
