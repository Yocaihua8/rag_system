...
> Scope：B-139 FastAPI + Uvicorn 运行时迁移边界；B-146 根目录结构收敛后的启动与静态服务边界
...
- B-146 后，静态首页只来自 Vue/Vite 构建产物 `backend/knowledge_island/static_dist/`；构建产物缺失时返回 503 构建提示，提示运行 `npm --prefix frontend run build`。
...
- 不修改 SQLite schema。
- 不迁移或删除 legacy `legacy/desktop/` 桌面端。
- 不改变导入、检索、回答、Agent 工具等业务规则。

## 4. 架构落点

FastAPI 只替换表现层 HTTP server 和路由适配方式。业务层仍由 `backend/knowledge_island/answers.py`、`backend/knowledge_island/search.py`、`backend/knowledge_island/ingestion.py`、`backend/knowledge_island/agent_tools.py` 等模块负责；数据层仍由 `backend/knowledge_island/storage.py` 作为 SQLite 唯一入口。
...
- `tests/backend/` 与 `tests/frontend/` 通过，或明确记录非 B-139 引起的既有失败。
