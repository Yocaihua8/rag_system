...
开发过程细节由 Git 提交和 Pull Request 保存；本文件只记录对使用者或维护者有意义的完成变更。
...
- **项目定向评估与覆盖率**：评估会话可按知识点或技能节点持久化发起，提交后记录规则或模型评分、置信度、命中证据、缺失点与来源，并聚合当前项目的知识覆盖和辅助技能状态。
- **版本化学习计划**：支持从项目差距生成新草稿、编辑、排序、确认和更新任务进度；重新生成不会覆盖已确认计划。
...
- **产品定位**：从通用本地 RAG 知识库收束为面向个人开发学习的本地项目知识教练；项目知识覆盖是主口径，语言、框架、数据、测试、交付和 AI 技能树仅作当前项目辅助口径。
- **评估状态**：统一为 `unassessed / needs_work / developing / mastered`，界面明确结果仅代表当前项目，不扩张为职业能力或求职评价。
- **兼容边界**：原导入、聊天和 `/api/assessment/*` 契约继续保留；新 Vue 主流程使用 Coach API，原 `/api/import/obsidian-vault` 继续作为一次性只读导入。
- **Codex 风格重排**：资料入口保持弹窗，设置保持全屏页；高级检索、模型比较和 Agent 审计继续收在高级区域。

### Fixed
- **Windows E2E 退出清理**：Playwright 完成浏览器用例后由测试专用全局 teardown 主动关闭 Uvicorn，避免命令在 Windows 进程树清理阶段挂起。
- **Tauri sidecar 静态契约**：测试对齐运行时 `.sidecar("knowledge-island-backend")` 与打包配置 `externalBin: ["binaries/knowledge-island-backend"]` 的不同职责。
...
### Added
- **Vue/Vite 前端工程骨架**：新增 `frontend/`、根 `package.json` 和 Vite 构建链；当时构建到后端托管目录，该耦合已由 B-168 移除
...
- **FastAPI 运行时**：Web MVP HTTP 服务层迁移到 FastAPI + Uvicorn，保留当时的根启动入口，并新增本地 `/docs` 自动接口文档入口；根入口已由 B-168 移除
...
### Changed
- **后端源码目录重组**：B-155 将原 `webapp/` 生产代码按职责迁移到 `backend/api`、`backend/routes`、`backend/domain`、`backend/storage`，删除受控 `webapp/` 源码目录；HTTP API 契约和 SQLite schema 保持不变，当时 Vue/Vite 仍输出到后端托管目录。
...
- **静态前端托管策略**：当时 FastAPI 只服务 Vite 构建产物；构建产物缺失时明确失败，不再回退 legacy static。该托管方案已由 B-168 取代
...
### Fixed
- **Docker 前端构建产物**：当时的单镜像 Docker 构建会生成并内置 Vue 产物，避免宿主机未预构建；该单镜像方案已由 B-168 的双服务取代。
...
### Changed
- 默认入口从 PySide6 桌面端切换为本地 Web MVP（`backend/__main__.py` → `webapp.server.run_server()`）；旧桌面端保留为 legacy
...
2. 只记录对使用者 / 维护者有意义的变更，开发过程细节由 Git 历史保存
