...
interface ProjectPageProps {
  ...
  insightError?: string
  creatingSourceFactReport?: boolean
  onCreateProject?: (name: string, rootPath: string) => void | Promise<void>
  onSelectProject?: (projectId: string) => void
  onScanSources?: () => void | Promise<void>
  onCreateSourceFactReport?: () => void | Promise<void>
}

export function ProjectPage({ project, projects = [], loading = false, error, creating = false, sources = [], documents = [], sourcesLoading = false, scanningSources = false, sourcesError, insight, insightLoading = false, insightError, creatingSourceFactReport = false, onCreateProject, onSelectProject, onScanSources, onCreateSourceFactReport }: ProjectPageProps) {
    ...
    <div className="standard-page">
      ...
      {project ? (
        <div className="content-grid">
          ...
          <section className="content-section"><h2>资料事实报告</h2><p>生成时只读取已经保存到 v3 Documents 的内容，报告会固定资料快照和逐项证据；它不会重新读取项目目录、调用模型或生成评分。</p><button className="button button--secondary" type="button" disabled={!onCreateSourceFactReport || creatingSourceFactReport} onClick={() => void onCreateSourceFactReport?.()}>{creatingSourceFactReport ? '正在创建报告…' : '生成资料事实报告'}</button>{!onCreateSourceFactReport && insight?.status === 'source_required' ? <p className="inline-notice">请先完成资料扫描。</p> : null}</section>
        </div>
      ) : null}
