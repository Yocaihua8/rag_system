...
- [x] 完成验证、提交 B-141J，并更新本 plan 快照；B-141 保持 `doing`
...
- [x] Vue 资料库页提供“同步当前项目目录”按钮。
- [x] 未选择项目空间时，同步按钮禁用。
- [x] 目录同步请求调用 `POST /api/import`，请求体包含当前 `project_id`。
- [x] 未选择项目空间时，API helper 抛出“请先创建或选择项目空间”。
- [x] 同步成功后刷新当前项目文档列表和导入批次历史，并显示导入结果计数。
- [x] B-141J 不迁移导入预检、删除文档、文档集合、项目改名/删除、后端行为或数据库 schema。
...
- 2026-05-26：完整验证通过：`npm run build`、`tests/test_webapp` 299 passed、legacy 回归 179 passed、B-141J touched-file `git diff --check` 退出码 0；浏览器资料库页烟测通过，目录同步入口可见，控制台错误数为 0。

## 9. 状态快照

- **最后更新时间**：2026-05-26
- **进度**：已完成 7 / 7 项（见 § 3 勾选状态）
- **最新 commit**：`c66a367` — `docs: 同步 B-141J 目录同步说明`
- **代码状态**：分支 `fix/url-virtual-source-preserve`；B-141J 当前目录同步薄片已完成；存在大量既有未提交改动，未纳入本轮提交
- **下一步**：继续 B-141K 页面级业务迁移，可在导入预检、Workbench SSE 会话或设置页模型配置中选择下一个薄片
