...
- [ ] 后端摄入协调器：先写失败测试，再新增项目级导入锁，保证同项目串行、跨项目不互相阻塞。
- [ ] FastAPI 分发线程池：先写失败测试，再把 `/api/*` 同步业务分发移入线程池，避免导入阻塞事件循环。
...
- **最新 commit**：`N/A` — 尚未提交
- **代码状态**：`fix/b-08-concurrent-index`；存在用户未提交改动；B-08 plan 已创建
- **下一步**：后端摄入协调器：先写失败测试，再新增项目级导入锁，保证同项目串行、跨项目不互相阻塞。
- **续任务须知**：提交时避免暂存非 B-08 既有改动。
