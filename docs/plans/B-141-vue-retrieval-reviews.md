...
- [ ] 增加 Vue 源码契约红灯测试，覆盖检索复盘 API helper、`SearchDebugPanel` 保存/列表/详情/删除控件、`WorkbenchView` 透传和 `App.vue` 状态流。
