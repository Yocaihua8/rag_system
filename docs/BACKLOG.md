...
| B-148 | feature | First-Run Wizard（首次运行向导） | doing | P1 | M | Tauri MVP 1 | RAG 团队 | docs/design/new-architecture-design.md §23.6, docs/features/first-run-wizard.md | Ollama 检测 → 模型拉取（SSE 进度） → 创建第一个知识库；新增 `/api/ollama/status` 和 `/api/ollama/pull`；plan：docs/plans/B-148-first-run-wizard.md |
