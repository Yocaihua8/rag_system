...
3. **最小变更量**：`webapp/storage.py`（`KnowledgeStore` 类）封装全部 CRUD，业务层不感知存储实现细节
...
| 模块 | 内容 |
|------|------|
| `webapp/storage.py` | `KnowledgeStore.__init__` 初始化 SQLite 连接，执行 `CREATE TABLE IF NOT EXISTS` 与兼容迁移 |
...
- `webapp/storage.py`：`KnowledgeStore` 封装 15+ 张业务表 CRUD，含兼容迁移
