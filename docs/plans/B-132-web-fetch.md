...
- [ ] 后端核心抓取服务：TDD 新增 URL 校验、SSRF 拦截、robots.txt 判断、大小/类型限制、HTML 正文净化测试与实现。
- [ ] 后端导入接口：TDD 新增 `/api/import/web-fetch/preview` 与 `/api/import/web-fetch/commit`，确认入库使用独立 `web:` 虚拟来源和 `web_fetch` 批次类型。
...
- **最后更新**：2026-06-30 17:10
- **进度**：已完成 0 / 5 项（见 § 3 勾选状态）
- **最新 commit**：`7b816cc` — docs: 归档 BACKLOG 已完成任务
- **代码状态**：`fix/b-08-concurrent-index`；B-132 plan 已创建，待运行基线测试和 TDD 红灯
- **下一步**：后端核心抓取服务：TDD 新增 URL 校验、SSRF 拦截、robots.txt 判断、大小/类型限制、HTML 正文净化测试与实现。
