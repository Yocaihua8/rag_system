...
| 前端 | Vue 3 + Vite | Vue 3.5+ / Vite 7+ | B-141/B-142 已完成主要 Web UI 迁移；B-143 已移除 `webapp/static/` fallback |
...
- 表现层：`frontend/`（Vue 3 + Vite 工程源码）、`webapp/static_dist/`（生产构建产物）、`webapp/server.py`（FastAPI app、静态文件、SSE）、`webapp/api.py`（兼容分发、参数校验）
