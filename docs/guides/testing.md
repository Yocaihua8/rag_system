...
**本地等价验证**：若本机未安装 `act` 且当前任务不允许 push，可按 CI job 命令分别运行 `pytest tests/test_backend tests/test_webapp -q`、`scripts/check_docs_consistency.py`、`npm run build`、`npx playwright install chromium` 和 `npx playwright test`。这只能证明命令链本地可通过；真正的 GitHub-hosted status check 仍以 PR / `main` 上的 Actions 结果为准。
