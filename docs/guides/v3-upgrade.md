...
5. 建立独立 React 前端，通过 OpenAPI 类型连接真实 v3 后端；该平行入口已完成固定任务切片与用户高保真验收，正式入口切换仍需独立门禁。
...
- React 实现的三种主题、视觉 token、文案、键盘、响应式和危险动作已于 2026-08-03 获用户确认。
- 高保真验收允许后续真实联调另行立项，但不自动授权修改 Vue UI、切换 Tauri/Docker/根正式入口或删除旧前端。
- 原型使用的演示内容不能复制到运行时、测试服务或错误回退。
- 当前平行 React 应用只开放后端能够证明的能力；资料、洞察、导出、桌面存储和任意工作流执行继续显示不可用边界。
