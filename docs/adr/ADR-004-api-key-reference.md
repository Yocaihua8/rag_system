...
模型 Profile 的 `api_key_ref` 字段只存储**引用字符串**，不存储明文 Key。引用在 `webapp/model_profiles.py` 的 `resolve_api_key_ref()` 函数中于**运行时解析**。
...
| 模块 | 内容 |
|------|------|
| `webapp/model_profiles.py` | `ALLOWED_API_KEY_REFS` 白名单；`resolve_api_key_ref()` 运行时解析；`model_profile_payload()` 只返回 `has_api_key / api_key_source` |
| `webapp/models.py` | `ModelProfile.api_key_ref` 字段（`TEXT`，默认 `""`）|
| `webapp/storage.py` | `model_profiles` 表 `api_key_ref TEXT NOT NULL DEFAULT ''`；写入前通过 `model_profile_validation_error()` 校验引用合法性 |
...
- `webapp/model_profiles.py`：`ALLOWED_API_KEY_REFS` + `resolve_api_key_ref()` + `model_profile_payload()`
- `webapp/storage.py`：`model_profiles` 表 schema 含 `api_key_ref` 字段
