...
- [x] 重构功能规格与 guides，回流 Desktop、GitHub、Obsidian、Docker 和维护事实后删除旧专题目录。
- [x] 更新根 README、AGENTS、CONTRIBUTING、CHANGELOG、文档脚本、CI 与仓库契约测试。
- [x] 运行全部文档门禁和相关契约测试，确认无行为漂移，移除 B-170 并删除本 plan。
...
冲突扫描覆盖旧 plan 目录及仓库内其他 plan 目录；只发现计划生命周期说明，没有状态为 Active 或 Interrupted 的任务 plan。

## 6. 完成标准

- [x] 文档目录与模板职责一致，且目录内部保持扁平。
- [x] API 文档与当前 86 个路径、94 个 GET/POST 操作及 SSE 事件一致。
- [x] 数据文档与当前 37 张 SQLite 表及关键字段一致。
- [x] 功能规格区分可达、不可达、部分接线和未实现状态。
- [x] 文档索引、链接、占位符、元数据、旧路径与源码引用检查通过。
- [x] 相关 OpenAPI 与 repository 测试通过，`git diff --check` 通过。
- [x] 业务源码、HTTP API、SQLite Schema 和 Agent 权限没有行为变化。
- [x] B-170 从 BACKLOG 移除，完成事实已写入 `CHANGELOG.md`，本 plan 将在下一提交删除。

## 7. 回流清单

| 内容 | 目标文档 | 是否完成 |
|------|----------|----------|
| 产品背景、目标用户、功能边界、用例和维护版本范围 | `docs/requirements/` | [x] |
| 完整系统、API、数据、安全、权限、状态和 UI 契约 | `docs/design/` | [x] |
| 当前用户可感知能力及可达性边界 | `docs/features/` | [x] |
| Desktop、GitHub、Obsidian、Docker、维护与排障操作 | `docs/features/`、`docs/guides/` | [x] |
| 新目录、命令、治理流程和发布可见变更 | 根文档、`docs/README.md`、`CHANGELOG.md` | [x] |
...
- 2026-08-01：文档检查迁至 `scripts/`，新增 API/Schema、依赖命令、源码文档引用与最近一级索引契约；仓库测试使用工作区内独立 basetemp，规避系统 pytest 临时目录权限异常。
- 2026-08-01：交付审查补齐模板行尾空白、计划完成规则和禁止直推 `main` 的合并门槛；当前提交重新执行 518 项 Python 测试、92 项前端单测、前端构建和三项文档门禁均通过。

## 9. 状态快照

- **最后更新**：2026-08-01
- **进度**：已完成 6 / 6 项（见 § 3 勾选状态）
- **最新 commit**：`430d5db` — `docs: 完成 B-170 文档体系重构`
- **代码状态**：`docs/template-canonical-restructure`；B-170 文档回流与契约已完成，本完成快照和治理门槛修正待提交。
- **下一步**：提交本完成快照，随后按完成流程删除本 plan。
