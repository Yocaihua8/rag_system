...
| 前端 | Vue 3 + Vite | Vue 3.5+ / Vite 7+ | B-144 后只服务 `backend/webapp/static_dist/` 构建产物，不再保留 legacy fallback |
| 测试框架 | pytest | 7+ | — |
...
| `backend/webapp/` | Web MVP 后端模块（API、检索、回答、导入、存储） |
...
- 表现层：`frontend/`（Vue 3 + Vite 工程源码）、`backend/webapp/static_dist/`（本地构建产物，不入库）、`backend/webapp/server.py`（FastAPI app、静态文件、SSE）、`backend/webapp/api.py`（兼容分发、参数校验）
- 业务层：`backend/webapp/answers.py`（回答生成）、`backend/webapp/search.py`（检索）、`backend/webapp/ingestion.py`（导入管线）、`backend/webapp/agent_tools.py`（Agent 工具）
- 数据层：`backend/webapp/storage.py`（SQLite 唯一读写入口）

**关键边界**：

- `api.py` 只做参数校验和用例编排，不直接操作 SQLite
- `storage.py` 是 SQLite 唯一入口，不承载业务规则
- Agent 工具只允许只读操作，白名单硬编码在 `backend/webapp/agent_tools.py`（ADR-003）
...
- 每个 `backend/webapp/` 模块文件只承载一个清晰职责（api / storage / search / answers / ingestion 各管一件事）
- 可选依赖（pymupdf / jieba）必须用 `try/except ImportError` 引入，失败时提供明确降级
- Vue 前端源码放在 `frontend/src/`，生产构建输出到 `backend/webapp/static_dist/`，不得提交 `node_modules/` 或构建产物
...
- 删除或重命名已有公共 API 接口
- 修改数据库 Schema（`backend/webapp/storage.py` 中的 `CREATE TABLE` / `ALTER TABLE`）
