...
构建产物输出到 `backend/knowledge_island/static_dist/`，该目录不入库。`python backend/app.py` 只服务 `backend/knowledge_island/static_dist/`；未构建或构建产物缺失时，首页会返回 503 构建提示，需要先运行 `npm --prefix frontend run build`。
...
- 通过 Docker 多阶段构建生成 Vue/Vite 前端产物。
...
- 执行 `docker compose --project-directory . -f ops/docker/compose.yaml up --build -d`。
...
```powershell
docker compose --project-directory . -f ops/docker/compose.yaml up --build -d
docker compose --project-directory . -f ops/docker/compose.yaml logs -f web
docker compose --project-directory . -f ops/docker/compose.yaml down
```
...
- Web MVP 设置默认模型 Profile 后，`/api/answer` 会优先使用该 Profile 的 provider、API 地址、模型名、温度、最大 tokens 和 Key 引用；没有默认 Profile 时继续使用现有单配置。
