...
- `frontend/src/api/settings.js` 封装 Vue 设置页基础模型设置和模型 Profile 入口，调用既有 `GET/POST /api/settings/llm`、`POST /api/settings/llm/test` 与 `/api/model-profiles*` 契约
...
- `frontend/src/views/SettingsView.vue` 是 B-141R 的设置页模型配置薄片，负责基础 LLM 设置读取/保存/测试和模型 Profile 列表、新建/编辑、删除、默认切换、连接测试入口
...
| Vue 3 / Vite | 必需前端构建依赖 | B-141 起的前端工程化和生产构建 | B-141A 提供骨架，B-141C/D/E/F/G/H/I/J/K/L/M/N/O/P/Q/R 已迁移项目空间选择/创建/改名/删除、非流式问答、文档浏览/删除、轻量导入、批次历史、普通文件上传、浏览器文件夹上传、当前目录同步、导入预检、文档集合筛选/新建/删除/重命名/加入/移出和设置页模型配置薄片；完整业务 UI 仍在迁移中 |
...
| Vue 3 + Vite 替代 Vanilla JS | 已采用（B-141A 起分阶段迁移）| ADR-006；先建立工程骨架和构建链，B-141C/D/E/F/G/H/I/J/K/L/M/N/O/P/Q/R 已迁移项目空间选择/创建/改名/删除、非流式问答、文档浏览/删除、轻量导入、批次历史、普通文件上传、浏览器文件夹上传、当前目录同步、导入预检、文档集合筛选/新建/删除/重命名/加入/移出和设置页模型配置薄片，完整业务 UI 后续按页面迁移 |
