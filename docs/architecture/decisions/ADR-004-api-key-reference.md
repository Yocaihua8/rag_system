...
> Date：2026-05-26
> Owner：RAG 团队
> Related：`../overview.md §5`、`../../product/features/authentication.md`、`ADR-005-remote-auth.md`
...
5. **兼容 Docker 部署**：Docker Compose 通过 `environment:` 注入 Key，与 `env:*` 引用天然对齐；无需在 Compose 文件中把 Key 写入 volume
...
3. 对应文档（`.env.example`、`docs/operations/setup.md`）
